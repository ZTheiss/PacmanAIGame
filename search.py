        ...
        for next in problem.getSuccessors(node):
            ...
            if nextState not in path:
                if problem.isGoalState(nextState):
                    ...
                    stack.push((nextState, actions+[n_direction]))
        ...
        if node not in path:
            ...
            for action in problem.getSuccessors(node):
                ...
                if problem.isGoalState(child):
                    return move + [n_direction]
                else:
                    frontier.push((child, move+[n_direction]))
    util.raiseNotDefined()
    ...
    while frontier.isEmpty() == 0:
        ...
        for action in problem.getSuccessors(node):
            ...
            if child not in path:
                frontier.update((child, move+[n_direction]), distance)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while frontier.isEmpty() == 0:
        ...
        for action in problem.getSuccessors(node):
            ...
            distance = action[2]
            if child not in path:
                frontier.update((child, move+[n_direction]), (problem.getCostOfActions(move+[n_direction])+heuristic(child, problem)))
            elif child in frontier.heap:
                frontier.update((child, move+[n_direction]), (problem.getCostOfActions(move+[n_direction])+heuristic(child, problem)))
